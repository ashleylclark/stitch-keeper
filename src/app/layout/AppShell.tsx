              ...
              <NavLink
                ...
                className={({ isActive }) =>
                    [
                      ...
                      isActive
                        ? 'bg-accent-200 text-stone-900 shadow-lg shadow-stone-900/10 dark:bg-accent-800 dark:text-stone-50 dark:shadow-black/20'
                        : 'text-stone-600 hover:bg-accent-50 hover:text-stone-900 dark:text-stone-300 dark:hover:bg-stone-800 dark:hover:text-stone-100',
                  ].join(' ')
                }
              >
                {({ isActive }) => (
  ...
  if (authSettings.oidcEnabled && !shouldSkipOidcRedirect) {
    return (
      <section className="mx-auto flex w-full max-w-5xl flex-col gap-4">
        <p className="text-xs font-semibold uppercase tracking-[0.35em] text-accent-600 dark:text-accent-300">
          Welcome
        </p>
      ...
      >
        ...
        <div className="flex flex-wrap gap-3">
          <button
            ...
            className="inline-flex h-12 items-center gap-2 rounded-2xl bg-accent-500 px-5 text-sm font-semibold text-white shadow-sm transition hover:bg-accent-600 disabled:cursor-not-allowed disabled:opacity-60 dark:bg-accent-700 dark:hover:bg-accent-600"
          >
            {isRegistering ? (
  ...
  return (
    <div className="min-h-screen bg-[linear-gradient(180deg,#fffaf7_0%,#f7f1eb_100%)] text-stone-900 dark:bg-[linear-gradient(180deg,#1c1917_0%,#292524_100%)] dark:text-stone-100">
      <div className="mx-auto flex min-h-screen w-full max-w-7xl gap-6 px-4 py-4 sm:px-6 lg:max-w-[1500px] lg:px-8">
        <aside className="hidden w-80 shrink-0 overflow-hidden rounded-[2rem] border border-white/70 bg-accent-50/95 shadow-[0_30px_80px_-40px_rgba(41,37,36,0.4)] backdrop-blur dark:border-stone-800 dark:bg-stone-900/95 dark:shadow-[0_30px_80px_-40px_rgba(0,0,0,0.65)] lg:block">
          <Sidebar />
        </aside>
        ...
        <div className="flex min-h-[calc(100vh-2rem)] flex-1 flex-col rounded-[2rem] border border-white/70 bg-white/55 shadow-[0_30px_80px_-40px_rgba(41,37,36,0.35)] backdrop-blur dark:border-stone-800 dark:bg-stone-950/70 dark:shadow-[0_30px_80px_-40px_rgba(0,0,0,0.7)]">
          <header className="flex items-center justify-between gap-4 border-b border-stone-200/70 px-5 py-4 dark:border-stone-800 sm:px-8 lg:px-10">
            <p className="text-xs font-semibold uppercase tracking-[0.25em] text-accent-600 dark:text-accent-300 sm:text-sm sm:tracking-[0.35em]">
              Stitch Keeper
            </p>
            <div className="flex items-center gap-3">
              {authStatus === 'authenticated' && session ? (
                <div className="hidden items-center gap-3 rounded-2xl border border-stone-200 bg-white px-3 py-2 text-sm text-stone-700 shadow-sm dark:border-stone-700 dark:bg-stone-900 dark:text-stone-200 sm:flex">
                  <span className="flex h-8 w-8 items-center justify-center rounded-xl bg-accent-50 text-accent-600 dark:bg-stone-800 dark:text-accent-300">
              ...
              {authStatus === 'unauthenticated' ? (
                <a
                  href="#login"
                  className="inline-flex h-11 items-center gap-2 rounded-2xl border border-accent-200 bg-accent-500 px-4 text-sm font-semibold text-white shadow-sm transition hover:bg-accent-600 dark:border-accent-700 dark:bg-accent-700 dark:hover:bg-accent-600"
                >
                  <LogIn color="currentColor" size={18} />
                  <span>Log in</span>
                </a>
              ...
              {authStatus === 'authenticated' ? (
                <button
                  ...
                  className="inline-flex h-11 w-11 items-center justify-center rounded-2xl border border-stone-200 bg-white text-stone-700 shadow-sm transition hover:border-accent-200 hover:text-stone-900 dark:border-stone-700 dark:bg-stone-900 dark:text-stone-200 dark:hover:border-accent-400 dark:hover:text-stone-50"
                >
          ...
          <div className="h-full w-[18.5rem] max-w-[85vw] border-l border-white/70 bg-[#f6ede6] shadow-2xl dark:border-stone-800 dark:bg-stone-900">
            <div className="flex items-center justify-end px-4 py-4">
              <button
                ...
                aria-label="Close navigation menu"
                onClick={() => setIsMobileMenuOpen(false)}
                className="inline-flex h-11 w-11 items-center justify-center rounded-2xl border border-stone-200 bg-white text-stone-700 shadow-sm transition hover:border-accent-200 hover:text-stone-900 dark:border-stone-700 dark:bg-stone-950 dark:text-stone-200 dark:hover:border-accent-400 dark:hover:text-stone-50"
              >
